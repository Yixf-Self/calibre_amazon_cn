    ...
    def __init__(self, url, result_queue, browser, log, relevance, domain,
        ...
        self.pd_xpath = '''
            //h2[text()="Product Details" or \
                 ...
                 starts-with(text(), "登録情報") or \
                 starts-with(text(), "基本信息")]/../div[@class="content"]
            '''
        ...
        self.publisher_xpath = '''
            descendant::*[starts-with(text(), "Publisher:") or \
                    ...
                    starts-with(text(), "出版社:") or \
                    starts-with(text(), "出版社:")]
            '''
        self.publisher_names = {'Publisher', 'Uitgever', 'Verlag', 'Editore', 'Editeur', 'Editor', 'Editora', '出版社', '出版社'}
        ...
        self.ratings_pat = re.compile(
            r'([0-9.]+) ?(out of|von|van|su|étoiles sur|つ星のうち|de un máximo de|de|平均) ([\d\.]+)( (stars|Sternen|stelle|estrellas|estrelas|sterren|星)){0,1}')
    ...
    def parse_pubdate(self, pd):
        for x in reversed(pd.xpath(self.publisher_xpath)):
            if x.tail:
                from calibre.utils.date import parse_only_date
                ans = x.tail
                date = ans.rpartition('(')[-1].replace(')', '').replace('年', '/').replace('月', '/').replace('日', '/').strip()
                date = self.delocalize_datestr(date)
...
class Amazon(Source):
    ...
    name = 'Amazon China'
    description = _('Downloads metadata and covers from Amazon China')
    ...
    def fetch_raw(self, log, url, br, testing,  # {{{
        ...
        if found:
            try:
                ...
                msg = 'Failed to parse amazon page for query: %r'%url
                log.exception(msg)
                return msg
    ...
    def identify(self, log, result_queue, abort, title=None, authors=None,  # {{{
        ...
        # link out of stock refinementLink 包含无库存商品
        stock_xpath = '''
            //h2[text()="Product Details" or \
                 text()="Produktinformation" or \
                 text()="Dettagli prodotto" or \
        ...
        # query out of stock books too
        try:
            stock_a = root.xpath(stock_xpath)
            if stock_a:
                stock_url = stock_a[0].get('href')
                if stock_url.startswith('/'):
                    stock_url = 'http://www.amazon.%s%s' % (self.get_website_domain(domain), stock_url)

                found, root = self.fetch_raw(log, query, br, testing)
        except  Exception as e:
            log.info("Can not parse stock link")
            log.exception(e)
